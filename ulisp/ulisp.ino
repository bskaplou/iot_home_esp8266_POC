...
// Lisp Library
const char LispLibrary[] PROGMEM = "(defun pri (&rest lst) (terpri) (mapc 'princ lst))";
...
#define ul_mark(x)         (car(x) = (object *)(((uintptr_t)(car(x))) | MARKBIT))
...
enum token { UL_UNUSED, BRA, KET, QUO, DOT };
...
INVERTDISPLAY, LOADCONFIG, SAVECONFIG, TEMPREAD, HUMIREAD, LIGHTREAD, CO2READ, TVOCREAD, DISCOVERY, 
SHOW, REQUEST, PERIODIC, ENDFUNCTIONS };
...
#define BUFFERSIZE 34  // Number of bits+2
  ...
  #define WORKSPACESIZE 2000-SDSIZE       /* Cells (8*bytes) */
...
char result_buff[RESULT_SIZE];
uint16_t result_idx;

void result_store(char c) {
  if(result_idx < RESULT_SIZE) {
    result_buff[result_idx++] = c;
  }
}

object *fn_saveconfig(object *args, object *env) {
  char buff[512];
  fs_saveconfig(cstring(first(args), buff, 512));
  return nil;
}

object *fn_loadconfig(object *args, object *env) {
  const char* data = fs_loadconfig();
  return lispstring(data);
}

object *fn_tempread(object *args, object *env) {
  return makefloat(ulisp_temperature_read());
}

object *fn_humiread(object *args, object *env) {
  return makefloat(ulisp_humidity_read());
}

object *fn_lightread(object *args, object *env) {
  return number(ulisp_light_read());
}

object *fn_co2read(object *args, object *env) {
  return number(ulisp_co2_read());
}

object *fn_tvocread(object *args, object *env) {
  return number(ulisp_tvoc_read());
}

object *fn_discovery(object *args, object *env) {
  (void) args, (void) env;
  object* prev = NULL;
  for(char** flist = ulisp_discovery(); *flist != NULL; flist++) {
    Serial.println(*flist);
    prev = cons(lispstring(*flist), prev);
  }
  return prev;
}

object *fn_show(object *args, object *env) {
  char buff[512];
  object* message_obj = first(args);
  cstring(message_obj, buff, 512);

  object* duration_obj = second(args);
  uint32_t duration = 5000;
  if(integerp(duration_obj)) {
    duration = duration_obj->integer;
  }

  ulisp_message_set(buff, duration);
  return nil;
}

object *fn_request(object *args, object *env) {
  char host[64];
  object* host_obj = first(args);
  cstring(host_obj, host, 64);

  object* port_obj = second(args);
  uint32_t port = 54321;
  if(integerp(port_obj)) {
    port = port_obj->integer;
  }

  char* callback;
  object* callback_obj = fourth(args);
  char* tmp = ulisp_to_string(callback_obj);
  uint16_t len = strlen(tmp) + 1;
  callback = (char*) malloc(len);
  memcpy(callback, tmp, len);

  object* message_obj = third(args);
  char* message = ulisp_to_string(message_obj);

  ulisp_request(host, port, message, callback);

  return nil;
}

object *fn_periodic(object *args, object *env) {
  object* period_obj = first(args);
  uint32_t period = 10000;
  if(integerp(period_obj)) {
    period = period_obj->integer;
  }

  object* code_obj = second(args);
  char* code = ulisp_to_string(code_obj);

  ulisp_periodic_task(period, code);
...
const char string_ext1[] PROGMEM = "load-config";
const char string_ext2[] PROGMEM = "save-config";
const char string_ext3[] PROGMEM = "temperature-read";
const char string_ext4[] PROGMEM = "humidity-read";
const char string_ext5[] PROGMEM = "light-read";
const char string_ext6[] PROGMEM = "co2-read";
const char string_ext7[] PROGMEM = "tvoc-read";
const char string_ext8[] PROGMEM = "discovery";
const char string_ext9[] PROGMEM = "show";
const char string_ext10[] PROGMEM = "request";
const char string_ext11[] PROGMEM = "periodic";
...
const tbl_entry_t lookup_table[] PROGMEM = {
  ...
  { string_ext1, fn_loadconfig, 0x00 },
  { string_ext2, fn_saveconfig, 0x11 },
  { string_ext3, fn_tempread, 0x00 },
  { string_ext4, fn_humiread, 0x00 },
  { string_ext5, fn_lightread, 0x00 },
  { string_ext6, fn_co2read, 0x00 },
  { string_ext7, fn_tvocread, 0x00 },
  { string_ext8, fn_discovery, 0x00 },
  { string_ext9, fn_show, 0x12 },
  { string_ext10, fn_request, 0x44 },
  { string_ext11, fn_periodic, 0x22 },
};
...
void ulisp_setup() {
  ...
  pfstring(PSTR("\nuLisp 3.3 ready to grok\n"), pserial);
}

char* ulisp_to_string(object* o) {
  result_idx = 0;
  printobject(o, result_store);
  //Serial.println("after print");
  result_buff[result_idx] = '\0';
  //Serial.println("after zero");
  return result_buff;
}

char* ulisp_run(char* code) {
  if(setjmp(exception) != 0) {
    return "error";
  } else {
    gc(NULL, NULL);
    //Serial.println("code to object*");
    object* lisp_code = lispstring(code);
    //Serial.println("code to object* done");
    GlobalString = lisp_code;
    GlobalStringIndex = 0;
    //Serial.println("read");
    object* prg = read(gstr);
    //Serial.println("read done");
    push(prg, GCStack);
    //Serial.println("after push");
    object* result = eval(prg, NULL);
    //Serial.println("after eval");
    pop(GCStack);
    //Serial.println("after pop");

    result_idx = 0;
    printobject(result, result_store);
    //Serial.println("after print");
    result_buff[result_idx] = '\0';
    //Serial.println("after zero");
    return result_buff;
